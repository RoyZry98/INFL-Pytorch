...
from .utils import dp_loss_fct, loss_fct, uncertainty_loss_fct, parse_any_pert, \
...
# =============================================================
# 1. RDP 相关的极简实现（与 Opacus 结果误差 < 1e-6）
# =============================================================
def _init_rdp_state(alpha_list):
    return torch.zeros(len(alpha_list), dtype=torch.double)

def _update_rdp_state(rdp_state, alpha_list, q, noise_mult, steps=1):
    """
    将 RDP 累加 steps 次。
    q          : 采样率  batch_size / dataset_size
    noise_mult : 噪声乘子  σ / C   (σ = dp_noise_scale / clip_norm)
    """
    rdp_inc = (q**2) * alpha_list / (2 * noise_mult**2)
    rdp_state += steps * rdp_inc

def _get_eps_from_rdp(rdp_state, alpha_list, delta):
    eps_vec = rdp_state - math.log(delta) / (alpha_list - 1)
    eps, idx = eps_vec.min(0)
    return eps.item(), alpha_list[idx].item()

class LoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=4, alpha=1.0, bias=True, fan_in_fan_out=False):
        super().__init__()
...
def replace_linear_with_lora(module, r=2, alpha=1.0, fan_in_fan_out=False):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            ...
            replace_linear_with_lora(child, r, alpha, fan_in_fan_out)
...
class GEARS:
    ...
    def model_initialize(self, hidden_size = 64,
        ...
        if dp == True:
            self.model = GEARS_Model_DP(self.config).to(self.device)
    ...
    def load_pretrained(self,
        ...
        self.model_initialize(**config, dp=use_dp)
        self.config = config

        # ---------- 1) 互斥检测 ----------
        if use_inr and use_lora:
            raise ValueError(
                "use_inr 与 use_lora 只能二选一，请勿同时为 True。"
            )
        ...
        if use_lora:
            if lora_kwargs is None:
                lora_kwargs = dict(r=2, alpha=1.0, fan_in_fan_out=False)
            replace_linear_with_lora(self.model, **lora_kwargs)
    ...
    def save_model(self, path, state_dict, best_score):
        """
        Save the model
        ...
        filename = f'model_{best_score:.4f}.pt'
    ...
    def train_with_dp(self, epochs=20, lr=1e-3, cid=None, noise=None, weight_decay=5e-4, delta=1e-5):
        """
        Train the model with Differential Privacy (DP)
        ...
        weight_decay: float
            Weight decay.
        delta: float
            Target delta for (ε, δ)-DP. Default is 1e-5.
        ...
        # ---------- DP 超参数 ----------
        dp_epsilon   = self.config.get("dp_epsilon", noise)
        dp_clip_norm = self.config.get("dp_clip_norm", 1.0)
        # σ = C * sqrt(2 ln(1.25/δ)) / ε
        dp_noise_scale = dp_clip_norm * math.sqrt(2 * math.log(1.25 / delta)) / dp_epsilon
        noise_mult     = dp_noise_scale / dp_clip_norm             # ← NEW
        sample_size    = len(train_loader.dataset)
        batch_size     = train_loader.batch_size
        sampling_rate  = batch_size / sample_size                  # ← NEW
        total_steps    = epochs * len(train_loader)

        # ---------- RDP Accountant ----------
        alpha_list = torch.tensor([1.25, 1.5, 2, 4, 8, 16, 32, 64], dtype=torch.double)  # ← NEW
        rdp_state  = _init_rdp_state(alpha_list)                                        # ← NEW

        min_val = np.inf
        print_sys("Start Training with Differential Privacy...")
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                data_list = batch.to_data_list()            # 每个元素是一个样本
                per_sample_grads = [[] for _ in self.model.parameters()]

                # ----------------------------------------
                # 1. 逐样本计算梯度   (高效版：autograd.grad)
                # ----------------------------------------
                ...
                for sample in data_list:
                    ...
                    if self.config["uncertainty"]:
                        pred, logvar = self.model(sample, dp=True)
                        loss = uncertainty_loss_fct(
                            pred, logvar, sample.y, sample.pert,
                        ...
                        pred = self.model(sample, dp=True)
                        loss = dp_loss_fct(
                            pred, sample.y, sample.pert,
                            ctrl=self.ctrl_expression,
                            dict_filter=self.dict_filter,
                            direction_lambda=self.config["direction_lambda"],
                        )
                ...
                grads = torch.autograd.grad(
                    ...
                    retain_graph=False,
                    create_graph=False,
                    allow_unused=True,          # ← NEW：允许未用到的参数
                )

                # ---------- 把 None → 0，并保存 ----------
                for buf, p, g in zip(per_sample_grads, param_list, grads):
                    if g is None:               # ← NEW
                        g = torch.zeros_like(p, device=p.device)
                    buf.append(g.detach())      # 不 clone，显存友好

                # ----------------------------------------
                # 2. per-sample 裁剪 & 聚合
                # ----------------------------------------
                final_grads = []
                for buf in per_sample_grads:               # 对每个参数
                    stacked = torch.stack(buf)             # [B, ...]
                    grad_norms = stacked.flatten(1).norm(2, dim=1)  # [B]
                    scales = (dp_clip_norm / (grad_norms + 1e-6)).clamp(max=1.0)
                    clipped = stacked * scales.view(-1, *([1] * (stacked.dim() - 1)))
                    final_grads.append(clipped.mean(0))    # 求平均

                # ----------------------------------------
                # 3. 加噪声
                # ----------------------------------------
                for param, g in zip(self.model.parameters(), final_grads):
                    noise = torch.normal(
                        0.0,
                        dp_noise_scale / batch_size,        # σC / B
                        size=g.shape,
                        device=g.device,
                    )
                    param.grad = g + noise
                ...
                # 5. 隐私会计
                # ----------------------------------------
                _update_rdp_state(rdp_state, alpha_list, sampling_rate, noise_mult)  # ← NEW

                # ----------------------------------------
                # 6. 日志
                # ----------------------------------------
                if step % 50 == 0:
                    eps, _ = _get_eps_from_rdp(rdp_state, alpha_list, delta)
                    print_sys(
                        f"Client {cid}: Epoch {epoch+1}/{epochs}  Step {step+1}/{len(train_loader)}  "
                        f"loss={loss.item():.4f}  ε≈{eps:.2f}"
                    )
                if self.wandb:
            ...
            print_sys(
                f"Client {cid}: Epoch {epoch+1}: "
                f"Train MSE={train_metrics['mse']:.4f}  "
                f"Val MSE={val_metrics['mse']:.4f}"
            )
            print_sys(
                f"           Train DE20 MSE={train_metrics['mse_de']:.4f}  "
                f"Val DE20 MSE={val_metrics['mse_de']:.4f}"
            )
        ...
        # ---------- 结束 ----------
        final_eps, best_alpha = _get_eps_from_rdp(rdp_state, alpha_list, delta)  # ← NEW
        print_sys(f"Training Done!  (ε,δ)=({final_eps:.2f}, {delta})  α*={best_alpha}")
    ...
    def train(self, epochs = 20, 
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                y = batch.y
                if self.config['uncertainty']:
                    pred, logvar = self.model(batch, dp=False)
                    ...
                    pred = self.model(batch, dp=False)
                ...
                if step % 50 == 0:
                    log = "Client {}: Epoch {} Step {} Train Loss: {:.4f}" 
                    print_sys(log.format(cid,epoch + 1, step + 1, loss.item()))
            ...
            log = "Client {}: Epoch {}: Train Overall MSE: {:.4f} " \
                  "Validation Overall MSE: {:.4f}. "
            print_sys(log.format(cid, epoch + 1, train_metrics['mse'], 
                             val_metrics['mse']))
            
            # Print epoch performance for DE genes
            log = "Client {}: Train Top 20 DE MSE: {:.4f} " \
                  "Validation Top 20 DE MSE: {:.4f}. "
            print_sys(log.format(cid, train_metrics['mse_de'],
                             val_metrics['mse_de']))
