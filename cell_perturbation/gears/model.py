...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        # batchnorms
        self.bn_emb = nn.LayerNorm(hidden_size)
        self.bn_pert_base = nn.LayerNorm(hidden_size)
        self.bn_pert_base_trans = nn.LayerNorm(hidden_size)
        
        # uncertainty mode
        if self.uncertainty:
            self.uncertainty_w = MLP([hidden_size, hidden_size*2, hidden_size, 1], last_layer_act='linear')
        
    def forward(self, data, dp):
        """
        ...
        if self.no_perturb:
            ...
            if dp == False:
                ...
                import numpy as np
                # 保证 pert_idx 是二维 list
                if isinstance(pert_idx, (int, np.integer)):
                    pert_idx = [[pert_idx]]
                elif isinstance(pert_idx, np.ndarray):
                    if pert_idx.ndim == 0:  # 标量
                        pert_idx = [[pert_idx.item()]]
                    elif pert_idx.ndim == 1:
                        pert_idx = [pert_idx.tolist()]
                    else:
                        pert_idx = pert_idx.tolist()
                elif torch.is_tensor(pert_idx):
                    if pert_idx.ndimension() == 0:
                        pert_idx = [[pert_idx.item()]]
                    elif pert_idx.ndimension() == 1:
                        pert_idx = [pert_idx.tolist()]
                    else:
                        pert_idx = pert_idx.tolist()
                elif isinstance(pert_idx, list):
                    if len(pert_idx) == 0:
                        pert_idx = []
                    elif isinstance(pert_idx[0], (int, np.integer)):
                        pert_idx = [pert_idx]
                    # 否则本身就是二维
                else:
                    raise RuntimeError(f"Unexpected type for pert_idx: {type(pert_idx)}")
