...
def count_inr_params(model):
    # 累加所有 SAN/INR 参数数量
    if not hasattr(model, 'SAN'):
        return 0
    return sum(p.numel() for m in model.SAN.values() for p in m.parameters())

def count_gears_params(model):
    # 累加主干参数数量（不包括 SAN/INR 部分）
    return sum(p.numel() for n, p in model.named_parameters() if not n.startswith('SAN.'))
...
def update_top20_models(top20_models, mse_de, round_num, model_state_dict, max_models=20, save_dir=None):
    """
    更新top20模型列表，保持按mse_de升序排列
    
    Args:
        top20_models: 当前top20模型列表
        mse_de: 当前模型的mse_de值
        round_num: 当前轮次
    ...
    # 添加新模型
    top20_models.append((mse_de, round_num, model_state_dict))
    
    # 按mse_de升序排序
    top20_models.sort(key=lambda x: x[0])
    
    # 如果超过最大数量，删除最差的模型
    if len(top20_models) > max_models:
        # 删除最差的模型文件
        worst_mse, worst_round, _ = top20_models.pop()
        if save_dir:
            worst_model_path = os.path.join(save_dir, f'model_round_{worst_round}_mse_{worst_mse:.6f}.pth')
            if os.path.exists(worst_model_path):
                os.remove(worst_model_path)
                print(f'        └── 删除最差模型: Round {worst_round}, MSE_DE: {worst_mse:.6f}')
    
    return top20_models

def save_top20_models(top20_models, save_dir, current_round, current_mse_de, model_state_dict):
    """
    保存当前模型到top20列表，并保存所有top20模型到磁盘
    
    Args:
        top20_models: 当前top20模型列表
        save_dir: 保存目录
        current_round: 当前轮次
        current_mse_de: 当前mse_de值
        model_state_dict: 当前模型状态字典
    ...
    # 更新top20模型列表
    top20_models = update_top20_models(top20_models, current_mse_de, current_round, 
                                      model_state_dict, max_models=20, save_dir=save_dir)
    
    # 保存当前模型
    current_model_path = os.path.join(save_dir, f'model_round_{current_round}_mse_{current_mse_de:.6f}.pth')
    torch.save(model_state_dict, current_model_path)
    
    # 打印当前top20状态
    print(f'        └── 当前Top20模型数量: {len(top20_models)}')
    print(f'        └── 最佳MSE_DE: {top20_models[0][0]:.6f} (Round {top20_models[0][1]})')
    print(f'        └── 最差MSE_DE: {top20_models[-1][0]:.6f} (Round {top20_models[-1][1]})')
...
def get_args():
    ...
    p.add_argument('--noise', type=float, default=1.29,
                   help='Gaushttps://www.baidu.com/link?url=RJ7jhJIRMMEveXGLuQAIvlqmec_zO8QkFJZezbZBLMSx3w6Kn7K_CTxYzUY8VBr354V77MgetV686sHYGTvdMsHlX3kCPfTBy-R_vJN2mHQiV4EWztXGdGwmGJ_k9imaJ2blYpMXB0SJLvJ3iXnNtT-oCsaEF2CtoLVtvZEYaHS&wd=&eqid=87531e050296f3ae0000000668da0804sian noise multiplier σ')
...
if __name__ == '__main__':
    ...
    # ★ 新增：保存top20模型的数据结构
    top20_models = []  # 存储 (mse_de, round, model_state_dict) 的列表
    max_models = 20    # 最多保存20个模型
    ...
    for rnd in range(args.global_round):
        ...
        # ★★★  保存Top20模型  ★★★
        # ============================================================
        # 每轮都保存模型到top20列表（如果当前模型比最差的20个模型更好）
        current_mse_de = test_metrics['mse_de']
        save_dir = 'private_INFL_logs_5e-3'
        
        # 如果top20列表未满，或者当前模型比最差的模型更好，则保存
        if len(top20_models) < max_models or current_mse_de < top20_models[-1][0]:
            top20_models = save_top20_models(top20_models, save_dir, rnd + 1, 
                                           current_mse_de, gears_global.model.state_dict())
            print(f'        ✓ 模型已保存到Top20列表 (Round {rnd+1}, MSE_DE: {current_mse_de:.6f})')
        else:
            print(f'        └── 当前模型未进入Top20 (MSE_DE: {current_mse_de:.6f} >= {top20_models[-1][0]:.6f})')
        ...
        os.makedirs('private_INFL_logs_5e-3', exist_ok=True)
        df.to_csv('private_INFL_logs_5e-3/fed_test_metrics.csv', index=False)
        ...
        plt.savefig('private_INFL_logs_5e-3/test_mse_curve.png', dpi=300)
        plt.close()                               # 释放内存
        ...
        # plt.savefig('private_INFL_logs_5e-3/subgroup_mse_curve.png', dpi=300)
        # plt.close()
    ...
    # ============================================================
    print('\n' + '='*60)
    print('训练完成！Top20模型总结:')
    print('='*60)
    
    if top20_models:
        print(f'共保存了 {len(top20_models)} 个模型:')
        print('排名 | 轮次 | MSE_DE    | 模型文件')
        print('-' * 50)
        for i, (mse_de, round_num, _) in enumerate(top20_models, 1):
            model_filename = f'model_round_{round_num}_mse_{mse_de:.6f}.pth'
            print(f'{i:2d}   | {round_num:4d} | {mse_de:8.6f} | {model_filename}')
        
        print(f'\n最佳模型: Round {top20_models[0][1]}, MSE_DE: {top20_models[0][0]:.6f}')
        print(f'最差模型: Round {top20_models[-1][1]}, MSE_DE: {top20_models[-1][0]:.6f}')
