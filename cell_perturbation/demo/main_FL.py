...
def get_args():
    ...
    p.add_argument('--n_client', type=int, default=10)
...
if __name__ == '__main__':
    ...
    # ---------------- 数据准备（同单机用法） -----------------------------
    pdata = PertData('./data')
    pdata.load(data_name='adamson')
    pdata.prepare_split(split='simulation', seed=1)
    pdata.get_dataloader(batch_size=32)
    ...
    for rnd in range(args.global_round):
        ...
        # ★★★  判断是否刷新最好模型  ★★★
        # ============================================================
        # 计算本轮四个子集的平均 mse_de（忽略 nan）
        score_list = [log_subgroup[g][-1] for g in target_groups
                      if not np.isnan(log_subgroup[g][-1])]
        if len(score_list) > 0:
            cur_score = np.mean(score_list)
            print(f'    >>> Avg MSE_DE ({", ".join(target_groups)}): {cur_score:.4f}')

            if cur_score < best_score:
                best_score = cur_score
                best_round = rnd + 1
                save_dir   = f'Ada_FL_logs'
        ...
        os.makedirs('Ada_FL_logs', exist_ok=True)
        df.to_csv('Ada_FL_logs/fed_test_metrics.csv', index=False)
        ...
        plt.savefig('Ada_FL_logs/test_mse_curve.png', dpi=300)
        plt.close()                               # 释放内存
        ...
        plt.savefig('Ada_FL_logs/subgroup_mse_curve.png', dpi=300)
        plt.close()
